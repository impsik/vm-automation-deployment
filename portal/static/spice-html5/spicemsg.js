...
function VDAgentClipboardGrab(types)
{
    ...
    if (types instanceof ArrayBuffer)
    {
        var dv = new SpiceDataView(types);
        for (var at = 0; at + 4 <= types.byteLength; at += 4)
            this.types.push(dv.getUint32(at, true));
    }
}
...
VDAgentClipboardGrab.prototype =
{
    to_buffer: function(a, at)
    {
        at = at || 0;
        var dv = new SpiceDataView(a);
        for (var i = 0; i < this.types.length; i++, at += 4)
            dv.setUint32(at, this.types[i], true);
    },
    buffer_size: function()
    {
        return this.types.length * 4;
    }
}

function VDAgentClipboardRequest(value)
{
    if (value instanceof ArrayBuffer)
        this.type = new SpiceDataView(value).getUint32(0, true);
    else
        this.type = value;
}
...
VDAgentClipboardRequest.prototype =
{
    to_buffer: function(a, at)
    {
        new SpiceDataView(a).setUint32(at || 0, this.type, true);
    },
    buffer_size: function()
    {
        return 4;
...
function VDAgentClipboard(type, data)
{
    if (type instanceof ArrayBuffer)
    {
        var dv = new SpiceDataView(type);
        this.type = dv.getUint32(0, true);
        this.data = new Uint8Array(type.slice(4));
    }
    else
    {
        this.type = type;
        this.data = data instanceof Uint8Array ? data : new Uint8Array(data || 0);
    }
}
...
VDAgentClipboard.prototype =
{
    ...
    {
        at = at || 0;
        new SpiceDataView(a).setUint32(at, this.type, true);
        new Uint8Array(a, at + 4, this.data.byteLength).set(this.data);
    },
    buffer_size: function()
    {
...
function VDAgentEmptyMessage() {}

VDAgentEmptyMessage.prototype =
{
    to_buffer: function() {},
    buffer_size: function()
    {
