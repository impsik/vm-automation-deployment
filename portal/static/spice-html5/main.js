...
import { SpiceInputsConn, sendCtrlAltDel, sendText } from './inputs.js';
...
SpiceMainConn.prototype.process_channel_message = function(msg)
{
    ...
    if (msg.type == Constants.SPICE_MSG_MAIN_AGENT_DATA)
    {
        ...
        else if (agent_data.type == Constants.VD_AGENT_CLIPBOARD_GRAB)
        {
            ...
            if (grab.types.indexOf(Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT) !== -1)
            {
                this.send_agent_message(
                    Constants.VD_AGENT_CLIPBOARD_REQUEST,
                    new Messages.VDAgentClipboardRequest(Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT)
                );
            }
            return true;
        }
        ...
        {
            var request = new Messages.VDAgentClipboardRequest(agent_data.data);
            if (
                request.type == Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT &&
                typeof this.host_clipboard_text === "string"
            )
            {
                this.send_agent_message(
                    Constants.VD_AGENT_CLIPBOARD,
                    new Messages.VDAgentClipboard(
                        Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT,
                        new TextEncoder().encode(this.host_clipboard_text)
                    )
                );
            }
            return true;
        ...
        {
            var clipboard = new Messages.VDAgentClipboard(agent_data.data);
            if (clipboard.type == Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT)
            {
                this.guest_clipboard_text = new TextDecoder("utf-8").decode(clipboard.data);
                window.dispatchEvent(new CustomEvent("spice-clipboard", {
                    detail: { text: this.guest_clipboard_text }
                }));
            }
            return true;
        }
        ...
        {
            this.guest_clipboard_text = "";
            window.dispatchEvent(new CustomEvent("spice-clipboard-release"));
            return true;
        }
...
SpiceMainConn.prototype.announce_agent_capabilities = function(request)
{
    var caps = new Messages.VDAgentAnnounceCapabilities(request, (1 << Constants.VD_AGENT_CAP_MOUSE_STATE) |
                                                        ...
                                                        (1 << Constants.VD_AGENT_CAP_CLIPBOARD) |
                                                        (1 << Constants.VD_AGENT_CAP_CLIPBOARD_BY_DEMAND));
    this.send_agent_message(Constants.VD_AGENT_ANNOUNCE_CAPABILITIES, caps);
}

SpiceMainConn.prototype.set_host_clipboard = function(text)
{
    this.host_clipboard_text = text;
    this.send_agent_message(
        Constants.VD_AGENT_CLIPBOARD_GRAB,
        new Messages.VDAgentClipboardGrab([Constants.VD_AGENT_CLIPBOARD_UTF8_TEXT])
    );
}
